...
final class DraftPipelineIntegrationTests: XCTestCase {
    ...
    func testNewsletterGateStopsChainBeforeLLMCall() async throws {
        ...
        // Deliberately never wired into anything: the assertion below is that
        // no code path ever started it.
        _ = StubChatProvider(name: "stub-local", tokens: ["should never run"], onStart: { calledLLM.mark() })
