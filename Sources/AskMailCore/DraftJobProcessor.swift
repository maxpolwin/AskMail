...
public enum DraftJobProcessor {
        ...
        for job in pending + retryableFailed {
            ...
            do {
                ...
                if NewsletterClassifier.isAutoGenerated(headers: headers) {
                    RollingLog.shared.log("draft skip [auto-generated, RFC 3834] \(logLabel(email))", level: .debug)
                ...
                if NewsletterClassifier.isNoReplySender(email.sender) {
                    RollingLog.shared.log("draft skip [no-reply sender] \(logLabel(email))", level: .debug)
                    try draftStore.updateJobState(sourceID: job.sourceID, messageID: email.messageID,
                ...
                if SenderExclusion.isExcluded(email.sender, excluded: excludedSenders) {
                    RollingLog.shared.log("draft skip [user exclusion list] \(logLabel(email))", level: .debug)
                    try draftStore.updateJobState(sourceID: job.sourceID, messageID: email.messageID,
                                                  state: .newsletterSkipped, attempts: job.attempts,
                                                  lastError: nil, updatedAt: updatedAt)
                    continue
                ...
                switch verdict {
                case .newsletter(let reason):
                    RollingLog.shared.log("draft skip [newsletter: \(reason)] \(logLabel(email))", level: .debug)
                    try draftStore.updateJobState(sourceID: job.sourceID, messageID: email.messageID,
                                                  state: .newsletterSkipped, attempts: job.attempts,
                                                  lastError: nil, updatedAt: updatedAt)
                case .personal(let reason):
                    RollingLog.shared.log("draft eligible [personal: \(reason)] \(logLabel(email))", level: .debug)
                    try draftStore.updateJobState(sourceID: job.sourceID, messageID: email.messageID,
                                                  state: .eligible, attempts: job.attempts,
                                                  lastError: nil, updatedAt: updatedAt)
                case .ambiguous:
                    RollingLog.shared.log("draft classify failed [ambiguous verdict] \(logLabel(email))", level: .debug)
                    try draftStore.updateJobState(sourceID: job.sourceID, messageID: email.messageID,
                                                  state: .failed, attempts: job.attempts + 1,
                                                  lastError: "classification ambiguous", updatedAt: updatedAt)
                }
            } catch {
                RollingLog.shared.log("draft classify failed [\(error)] sourceID=\(job.sourceID)", level: .debug)
                try draftStore.updateJobState(sourceID: job.sourceID, state: .failed,
                                              attempts: job.attempts + 1,
    ...
    /// Sender + subject label for classification-decision log lines --
    /// mail metadata, so `.debug`-gated only (H-23: sender/subject are
    /// treated the same as chunk text/body content for the default `.info`
    /// log's data-minimization guarantee, even though this label itself
    /// carries no body text).
    private static func logLabel(_ email: IngestableEmail) -> String {
        "\(email.sender) | \(email.subject.prefix(80))"
    }
    ...
    public static func draftEligibleJobs(draftStore: DraftStore, askStore: SQLiteStore,
                                         ...
                                         concurrency: Int, accountEmail: String = "", now: Date = Date()) async {
            ...
            func launchNext() {
                ...
                group.addTask {
                    await draftOne(job: job, draftStore: draftStore, askStore: askStore,
                                   chatProvider: chatProvider, embedder: embedder,
                                   accountEmail: accountEmail, now: now)
                }
            }
    ...
    private static func draftOne(job: DraftJob, draftStore: DraftStore, askStore: SQLiteStore,
                                 chatProvider: ChatProvider, embedder: EmbeddingProvider,
                                 accountEmail: String, now: Date) async {
        let updatedAt = Int64(now.timeIntervalSince1970)
        do {
            ...
            let draftText = try await generateDraftText(thread: thread, draftStore: draftStore,
                                                         askStore: askStore, chatProvider: chatProvider,
                                                         embedder: embedder, accountEmail: accountEmail)
            ...
            try draftStore.updateJobState(sourceID: job.sourceID, state: .drafted,
                                          attempts: job.attempts, lastError: nil, updatedAt: updatedAt)
            RollingLog.shared.log("draft ready [thread=\(threadID) sender=\(latest.sender) subject=\(latest.subject.prefix(80))]",
                                  level: .debug)
        } catch {
            // Previously silent on failure -- every job-queue drafting
            // failure was only ever visible by querying draft_jobs.last_error
            // directly, not in the debug log a user would actually export.
            RollingLog.shared.log("draft failed [sourceID=\(job.sourceID) error=\(error)]", level: .info)
            try? draftStore.updateJobState(sourceID: job.sourceID, state: .failed,
                                           attempts: job.attempts + 1,
                                           lastError: String(describing: error), updatedAt: updatedAt)
        }
    ...
    /// On-demand regeneration for the macOS Services menu's "Regenerate"
    /// verb (Phase 4, docs/draft-modus-plan.md): bypasses the job queue and
    /// scheduler cadence entirely -- called directly from
    /// `DraftServiceProvider`, never from `runTick`. Always grounds on
    /// `threadID`'s current latest message (not any particular past job),
    /// reusing the exact same retrieval/style/assembly path `draftOne` uses
    /// via `generateDraftText` rather than duplicating it.
    public static func regenerateDraft(threadID: String, draftStore: DraftStore, askStore: SQLiteStore,
                                       chatProvider: ChatProvider, embedder: EmbeddingProvider,
                                       accountEmail: String = "", now: Date = Date()) async throws -> DraftRecord {
        let thread = try askStore.threadMessages(threadID: threadID)
        guard let latest = thread.last else { throw DraftJobError.emptyThread }
        let draftText = try await generateDraftText(thread: thread, draftStore: draftStore, askStore: askStore,
                                                     chatProvider: chatProvider, embedder: embedder,
                                                     accountEmail: accountEmail)
        let generatedAt = Int64(now.timeIntervalSince1970)
        // Only after generation succeeds: replace, not accumulate. A failed
        // generation must never delete the thread's existing ready draft.
        try draftStore.deleteReadyDrafts(threadID: threadID)
        let pk = try draftStore.insertDraft(threadID: threadID, latestMessageID: latest.messageID,
                                            sender: latest.sender, subject: latest.subject,
                                            draftText: draftText, generatedAt: generatedAt, status: .ready)
        return DraftRecord(pk: pk, threadID: threadID, latestMessageID: latest.messageID, sender: latest.sender,
                           subject: latest.subject, draftText: draftText, generatedAt: generatedAt, status: .ready)
    }

    /// Core generation step shared by the job-queue path (`draftOne`) and
    /// the on-demand Services-menu path (`regenerateDraft`): retrieval,
    /// learned style guidance, prompt assembly, and the chat stream.
    /// `thread`'s last element is the message the reply is grounded on.
    private static func generateDraftText(thread: [ThreadMessage], draftStore: DraftStore, askStore: SQLiteStore,
                                          chatProvider: ChatProvider, embedder: EmbeddingProvider,
                                          accountEmail: String) async throws -> String {
        guard let latest = thread.last else { throw DraftJobError.emptyThread }
        ...
        let assembled = DraftAssembler().assemble(thread: thread, grounding: grounding,
                                                  styleGuidance: styleGuidance, accountEmail: accountEmail)
        var draftText = ""
